...
def carrera_pdf_list(request):
    ...
    pdfsprimer = PDF.objects.none()
    pdfssegundo = PDF.objects.none()
    pdfstercero = PDF.objects.none()
    pdfscuarto = PDF.objects.none()
    pdfsquinto = PDF.objects.none()
    pdfssexto = PDF.objects.none()
    pdfsseptimo = PDF.objects.none()
    pdfsoctavo = PDF.objects.none()
    pdfsnoveno = PDF.objects.none()
    pdfsdecimo = PDF.objects.none()
    ...
    if selected_carrera_id:
        ...
        pdfsprimer = PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Primer", semestre__icontains="Primer")
        pdfssegundo = PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Primer", semestre__icontains="Segundo")  
        pdfstercero = PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Segundo", semestre__icontains="Primer") | PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Segundo", semestre__icontains="Tercer")
        pdfscuarto = PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Segundo", semestre__icontains="Segundo") 
        pdfsquinto = PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Tercero", semestre__icontains="Primer")
        pdfssexto = PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Tercero", semestre__icontains="Segundo")
        pdfsseptimo = PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Cuarto", semestre__icontains="Primer")
        pdfsoctavo = PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Cuarto", semestre__icontains="Segundo")
        pdfsnoveno= PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Quinto", semestre__icontains="Primero")
        pdfsdecimo = PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Quinto", semestre__icontains="Segundo") | PDF.objects.filter(id_carrera=selected_carrera, curso__icontains="Quinto", semestre__icontains="Décimo")
        
    return render(request, 'carrera_pdf_list.html',
                  {'carreras': carreras, 'pdfs': pdfs, 'selected_carrera_id': selected_carrera_id, 'pdfsprimer': pdfsprimer,'pdfssegundo': pdfssegundo, 'pdfstercero': pdfstercero, 'pdfscuarto': pdfscuarto, 'pdfsquinto': pdfsquinto, 'pdfssexto': pdfssexto, 'pdfsseptimo': pdfsseptimo, 'pdfsoctavo': pdfsoctavo, 'pdfsnoveno': pdfsnoveno, 'pdfsdecimo': pdfsdecimo})
...
def pdf_to_html(request):
    pdf_ids = request.POST.get('selected_pdfs')
    print(pdf_ids)
    if pdf_ids:
        pdf_ids_list = json.loads(pdf_ids)
        print(pdf_ids_list)
        pdf_instances = PDF.objects.filter(id__in=pdf_ids_list)
        html_contents = []
        ...
        return HttpResponse('<hr>'.join(html_contents))
    else:
